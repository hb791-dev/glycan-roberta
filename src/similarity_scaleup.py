...
def _iter_local_html_references(html_text: str) -> list[str]:
    """Return local `href` and `src` references found in one HTML document.

    The public export helper uses this to discover any extra local files a page
    depends on, such as a relative PNG added later to the top-level report.
    """
    local_references: list[str] = []
    for match in re.finditer(r"""(?:href|src)=['"]([^'"]+)['"]""", html_text, flags=re.IGNORECASE):
        reference = match.group(1).strip()
        if not reference:
            continue
        if reference.startswith(("#", "data:", "mailto:", "javascript:", "http://", "https://")):
            continue
        local_references.append(reference)
    return local_references


def _resolve_local_export_reference(
    source_path,
    reference: str,
    html_root_dir,
) -> Path | None:
    """Resolve one relative HTML reference to a real file inside the HTML root."""
    source_path = Path(source_path).resolve()
    html_root_dir = Path(html_root_dir).resolve()
    candidate_path = (source_path.parent / reference).resolve()
    try:
        candidate_path.relative_to(html_root_dir)
    except ValueError:
        return None
...
def _scan_text_for_sensitive_strings(
    ...
    """Collect obvious personal or local-environment strings from one text file.

    The goal is not perfect secret scanning. It is a practical last pass to
    catch the most common notebook-sharing mistakes before HTML is pushed to a
    public GitHub path.
    """
    scan_rows: list[dict[str, str]] = []
    default_patterns = (
        ("email_address", r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}"),
        ("colab_path", r"/content(?:/[^\s\"'<>]+)*"),
        ("google_drive_path", r"(?:/content/drive|MyDrive|/drive/MyDrive)(?:/[^\s\"'<>]+)*"),
        ("local_user_path", r"/Users/[^\s\"'<>]+"),
        ("file_url", r"file:///+[^\s\"'<>]+"),
    )

    for match_type, pattern in default_patterns:
        for match in re.finditer(pattern, text, flags=re.IGNORECASE):
            scan_rows.append(
                {
                    "file_name": source_name,
                    "match_type": match_type,
                    "match_text": match.group(0)[:160],
                }
            )

    for blocked_text in extra_blocked_strings or ():
        normalized_text = str(blocked_text).strip()
        ...
        if normalized_text in text:
            scan_rows.append(
                {
                    "file_name": source_name,
                    "match_type": "custom_blocked_string",
                    "match_text": normalized_text[:160],
                }
            )
...
def _strip_local_html_links(text: str) -> str:
    """Replace local HTML anchor tags with plain text for index-only exports."""
    ...
    def _replace(match: re.Match[str]) -> str:
        ...
        return f"{label} (report page omitted from index-only public export)"

    return anchor_pattern.sub(_replace, text)


def export_public_scaleup_html(
    ...
    repo_ref: str = "main",
    extra_blocked_strings: Sequence[str] | None = None,
    export_mode: str = "index_only",
) -> dict:
    """Copy a clean, browser-facing subset of notebook 8 HTML into one folder.
    ...
    if export_dir is None:
        raise ValueError("export_dir is required for export_public_scaleup_html().")
    if export_mode not in {"index_only", "full_report_set"}:
        raise ValueError(
            "export_mode must be either 'index_only' or 'full_report_set'."
        )
    ...
    while pending_relative_paths:
        ...
        for reference in _iter_local_html_references(text):
            ...
            if (
                ...
                and resolved_dependency.suffix.lower() in {".html", ".htm"}
            ):
                # In the fast export mode, keep only the main landing page plus
                # non-HTML assets it needs directly. This avoids copying and
                # rescanning every accession-specific report page.
                continue
    ...
    if export_mode == "index_only":
        exported_index_path = export_path / index_relative_path
        if exported_index_path.exists():
            exported_index_html = exported_index_path.read_text(encoding="utf-8")
            exported_index_path.write_text(
                _strip_local_html_links(exported_index_html),
                encoding="utf-8",
            )
